...
public final class HandEval {
     ...
     */
    public static long encode(final SBCard card) {
     ...
     */
    public static long encode(final SBCardSet cs) {
        long result = 0;
        for(SBCard c : cs) {
            result |= encode(c);
        }
    ...
    public static long encode(SBCard[] cs) {
        long result = 0;
    ...
    public static long encode(int[] cs) {
        long result = 0;
        for(int i = 0; i < cs.length; i++) //result |= (0x1L << (card.suitOf().ordinal()*13 + card.rankOf().ordinal()););
        {
            result |= (0x1L << (cs[i]));
        }
        return result;
...
// javac doesn't propagate NO_PAIR (==0) so doesn't constant-fold it out of bitwise-or expressions
    ...
    public static int hand8LowEval(long hand) {
        ...
        return loEvalOrNo8Low[ /* rotate each 13-bit suit field left to put Ace in LS bit */((((int) hand & 0x1FFF) << 1) + (((int) hand ^ 0x1000) >> 12)) | ((((int) hand >> 12) & 0x3FFE) + (((int) hand ^ (0x1000 << 13)) >> 25)) | (((int) (hand >> 25) & 0x3FFE) + (int) ((hand ^ (0x1000L << 26)) >> 38)) | (((int) (hand >> 38) & 0x3FFE) + (int) ((hand ^ (0x1000L << 39)) >> 51))];
    }
...
//	}
    ...
    public static void main(String[] args) {
        SBCard[] cs = new SBCard[]{new SBCard("As"), new SBCard("Ad"), new SBCard("3h"), new SBCard("3d"), new SBCard("3s")};
        long hand = HandEval.encode(cs);
        int rank = HandEval.hand5Eval(hand);
        System.out.println(Integer.toHexString(rank));


        hand = HandEval.encode(cs);
        rank = HandEval.hand5Eval(hand);

        int k;
        k = rank & FULL_HOUSE;
        k = rank & TWO_PAIR;

        cs = new SBCard[]{new SBCard("As"), new SBCard("2s"), new SBCard("9s"), new SBCard("4s"), new SBCard("3s")};
