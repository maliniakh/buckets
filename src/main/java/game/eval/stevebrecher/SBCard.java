...
public class SBCard {
	 ...
	 */
	public SBCard(Rank rank, Suit suit) {
		this.rank = rank;
		this.suit = suit;
	}
	...
	private final static SBCard[]	theCards = new SBCard[52];
	static {
		int i = 0;
		for (Suit s : Suit.values())
			for (Rank r : Rank.values())
				theCards[i++] = new SBCard(r, s);
	}
	 ...
	 */
	public static SBCard getInstance(Rank rank, Suit suit) {
	 ...
	 * @throws IllegalArgumentException on the first character in rs which is not found in the respective string.
	 */
	public static SBCard getInstance(String rs) {
		if (rs.length() != 2)
			throw new IllegalArgumentException('"' + rs + "\".length != 2");
		try {
	...
	@Override public boolean equals (Object that) {
		if (!(that instanceof SBCard)) return false;
		return this.rank == ((SBCard)that).rank && this.suit == ((SBCard)that).suit;
	}
